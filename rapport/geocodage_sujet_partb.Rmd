...
<button class="btn btn-primary" data-toggle="collapse" data-target="#BlockName_cartoNA"> Carto_correpsondance géocodage EVS </button>
...
Bien que ce soit un échantillon particulier il est interressant de regarder les erreurs que nous rencontrons : 

Puis voici une liste des adresses (9) trouvés par EVS mais fausses :

* ID 01_0437_6 : la géolocalisation est fausse comme le sugère `result_score` = .26

* ID 01_0598_6 : géoloclisation fausse (de pas bcp mais tout de meme), le chemin se trouve au bout de la route
 
* ID 02_0451_8 / 16_0447_4 : faux c'est une adresse à Lyon que le géocodeur à mis à Paris/Aix, il semble que c'est le cas pour toutes les villes à arrondissement. Le géocodeur d'ESRI cést planté car le nom de la ville est inversé avec celui de le CP
...
* ID 20-0755-5 : compter comme bon si c'est une fusion de communes à verifier l'adresse colle bien 
...
* 9 difficile à localiser (en dehors d'une precision à l'échelle de la commune)

Une piste pour ameliorer verifier les résultats semble être de comparer la commune en entré de géocodage de celle en sortie (cf. ID 02_0451_8 / 16_0447_4 par exemple).

### NA géocodage EVS


Il y a 888 valeurs manquantes dans le géocodage (il faut cependant garder ee tête que 197 a été deja fait à la main). De la meme manière que le paragraphe prècedent, on peut regarder comment ces dernierers ont éte codées par le geocodeur ESRI. 

```{r NAgeocodageEVS, fig.align='center'}
