...
* Garder traces des différentes modifications sur le jeux de données et des opérations pour le geocodage
...
Il va être important pour la base de données de clarifier cela et de bien considérer que l'on a bien une seule adresse mais distincte en dates.


Avant de les présenter nous pouvons regarder les sujets comportant trop de valeurs manquantes risquant de rendre tout geocodage impossible. 

## Valeurs manquantes du jeu de données.

Il y a `r nrow(allsujet_clean.dat[is.na(allsujet_clean.dat$Commune),])` communes d'adresses manquantes sur 46 sujets (un sujet possède 3 adresses manquantes). L'absence de commune correspond (sauf dans 3 cas et nous y reviendrons) à une absence totale d informations. 
